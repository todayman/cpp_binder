...
import manual_types;
import translate.types : determineStrategy, translateType, makeTypeForDecl, unresolvedTypes;
...
void populateDAST()
{
    ...
    if (unresolvedTypes.length > 0)
    {
        ...
        auto local_copy = unresolvedTypes.dup;
        foreach (type, decl; local_copy)
        {
            if (decl !is null)
            {
...
package void resolveType(unknown.Declaration cppDecl)
{
    ...
    makeTypeForDecl(cppDecl, name, mod.moduleDeclaration.moduleName, new IdentifierOrTemplateChain());
}
